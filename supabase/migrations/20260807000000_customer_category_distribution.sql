-- 20260807000000_customer_category_distribution.sql

-- 1. Add category distribution column to customers table
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS category_last_invoice_dates JSONB DEFAULT '{}'::jsonb;

-- 2. Drop the old RPC to redefine it with new fields
DROP FUNCTION IF EXISTS sync_customer_data(JSONB);

-- 3. Recreate the RPC function to handle all fields including category_last_invoice_dates
