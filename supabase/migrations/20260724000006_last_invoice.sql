-- Add new invoice columns to customers table
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS last_invoice_date TIMESTAMPTZ;
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS last_invoice_amount NUMERIC;
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS last_invoice_number TEXT;

-- Drop the old RPC to redefine it with new fields
DROP FUNCTION IF EXISTS sync_customer_data(JSONB);

-- Recreate the RPC function to handle all fields including invoice details
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.customers (
            ...
            last_invoice_date, last_invoice_amount, last_invoice_number, updated_at
        )
        VALUES (
            v_net_suite_id,
