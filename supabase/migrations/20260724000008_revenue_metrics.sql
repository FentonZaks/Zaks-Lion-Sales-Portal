-- 20260724000008_revenue_metrics.sql

-- 1. Add revenue columns to customers table
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS mtd_revenue NUMERIC;
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS ytd_revenue NUMERIC;

-- 2. Drop the old RPC to redefine it with new fields
DROP FUNCTION IF EXISTS sync_customer_data(JSONB);

-- 3. Recreate the RPC function to handle all fields including revenue
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.customers (
            ...
            mtd_revenue, ytd_revenue, updated_at
        )
        VALUES (
            v_net_suite_id,
